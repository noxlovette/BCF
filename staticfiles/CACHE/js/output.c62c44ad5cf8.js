let user_id=sessionStorage.getItem('user_id');function fetchIngredients(){fetch(`api/collection?$user_id=${user_id}`).then(response=>response.json()).then(data=>{const fragment=document.createDocumentFragment();data.results.forEach(collection_ingredient=>{const row=document.createElement('tr');row.innerHTML=`
