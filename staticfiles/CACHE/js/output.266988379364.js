let currentPage=1;let totalPages=1;let userId=sessionStorage.getItem('user_id');document.getElementById('prev-page').addEventListener('click',function(){if(currentPage>1){currentPage--;console.log(currentPage)
fetchIngredients();}});document.getElementById('next-page').addEventListener('click',function(){if(currentPage<totalPages){currentPage++;console.log(currentPage)
fetchIngredients();}});function addToCollection(ingredientId,userId){let data={user_id:userId,ingredient_id:ingredientId};fetch(`/collection/api/collection/${userId}`,{method:'POST',headers:{'Content-Type':'application/json',},body:JSON.stringify(data),}).then(response=>{if(!response.ok){throw new Error('Network response was not ok');}
...
return response.json();}).then(data=>{console.log(data);totalPages=data.total_pages;console.log(totalPages);const fragment=document.createDocumentFragment();data.results.forEach(ingredient=>{const row=document.createElement('tr');row.innerHTML=`
                    <td>
