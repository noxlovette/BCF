...
let inputElement;if(cellId==='is_collection'){inputElement=`<input class="collection-input" type="checkbox" ${cellText === 'true' ? 'checked' : ''}>`;}else if(cellId==='amount'){inputElement=`<input class="collection-input" type="number" value="${parseInt(cellText, 10)}">`;}else{inputElement=`<input class="collection-input" type="text" value="${cellText}">`;}
cell.innerHTML=inputElement;cell.id=cellId;console.log('cellId after conversion',cell.id)});const saveButton=createButton('Save','save',saveIngredient,id);const cancelButton=createButton('Cancel','cancel',fetchIngredients);const buttonCell=row.querySelector('.editing');buttonCell.appendChild(saveButton);buttonCell.appendChild(cancelButton);}
...
const key=input.id;console.log('key',key)
