...
function deleteCustomCollectionIngredient(customCollectionIngredientId,userId){console.log('Deleting custom collect ingredient:',customCollectionIngredientId);$.ajax({url:`/collection/api/ingredient/${userId}/custom/${customCollectionIngredientId}/delete/`,method:'DELETE',headers:{'X-CSRFToken':getCookie('csrftoken'),'Content-Type':'application/json',},success:function(response){fetchIngredients();},error:function(error){console.error('Error deleting ingredient:',error);}});}
function editIngredient(event,id,selectors,saveFunction){const row=event.target.parentNode.parentNode;row.dataset.id=id;console.log('row.dataset.id when edit has been clicked',row.dataset.id)
console.log('id when edit has been clicked',id)
const cells=row.querySelectorAll(selectors);cells.forEach((cell,index)=>{const cellText=cell.textContent;const cellId=cell.id;let inputElement;if(cellId==='is_collection'){inputElement=document.createElement('input');inputElement.className="collect-input";inputElement.type="checkbox";if(cellText==='true'){inputElement.checked=true;}}else if(cellId==='amount'){inputElement=document.createElement('input');inputElement.className="collect-input";inputElement.type="number";inputElement.value=parseInt(cellText,10);}else{inputElement=document.createElement('input');inputElement.className="collect-input";inputElement.type="text";inputElement.value=cellText;}
inputElement.id=`${cellId}-input`;cell.innerHTML='';cell.appendChild(inputElement);});const saveButton=createButton('Save','save',function(event){saveFunction(event,id);});const cancelButton=createButton('Cancel','cancel',fetchIngredients);const buttonCell=row.querySelector('.editing');buttonCell.appendChild(saveButton);buttonCell.appendChild(cancelButton);}
...
function saveIngredientCommon(event,url,id){console.log('Saving ingredient...')
console.log('id when save button has been clicked',id)
const row=document.querySelector(`tr[data-id="${id}"]`);const inputs=row.querySelectorAll('input.collect-input');const data={};inputs.forEach((input)=>{console.log('input',input)
...
function saveCollectionIngredient(event,id){const url=`/collection/api/ingredient/${userId}/${id}/update/`;saveIngredientCommon(event,url,id);}
function saveCustomCollectionIngredient(event,id){const url=`/collection/api/ingredient/${userId}/custom/${id}/update/`;saveIngredientCommon(event,url,id);}
function fetchIngredients(){console.log('Fetching ingredients...')
$.ajax({url:`api/collection/${userId}/`,method:'GET',success:function(data){let ingredients=Array.isArray(data)?data:[data];const tableBody=$('#collect-table tbody');tableBody.empty();ingredients.forEach(collection_ingredient=>{let ingredient=collection_ingredient.ingredient?collection_ingredient.ingredient:{};const rowHtml=`
                    <tr>
...
deleteIngredientButton.on('click',function(){let type=$(this).data('type');let id=$(this).data('id');if(type==='CollectionIngredient'){deleteCollectionIngredient(id,userId);}else if(type==='CustomCollectionIngredient'){deleteCustomCollectionIngredient(id,userId);}});let editIngredientButton=row.find('.edit');editIngredientButton.data('type',collection_ingredient.type);editIngredientButton.data('id',collection_ingredient.id);editIngredientButton.on('click',function(event){let type=$(this).data('type');if(type==='CollectionIngredient'){editCollectionIngredient(event,collection_ingredient.id);}else if(type==='CustomCollectionIngredient'){editCustomCollectionIngredient(event,collection_ingredient.id);}});});},error:function(error){console.error('Error fetching ingredients:',error);}});}
$(document).ready(function(){$('.btn-create-ingredient').on('click',createCustomIngredient);});function createCustomIngredient(){const rowHtml=`
        <tr>
