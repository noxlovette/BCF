...
function editCollectionIngredient(event,id){const row=event.target.parentNode.parentNode;row.dataset.id=id;console.log('row id after assignment, edit button',row.dataset.id)
const cells=row.querySelectorAll('td.editable');cells.forEach((cell,index)=>{const cellText=cell.textContent;const cellId=cell.id;cell.innerHTML=`<input class="collection-input" type="text" value="${cellText}">`;cell.id=`${cellId}-input`;});console.log('row id when edit button has been clicked',row.dataset.id)
