...
function editCollectionIngredient(event,id){const row=event.target.parentNode.parentNode;const cells=row.querySelectorAll('td.editable');cells.forEach((cell,index)=>{const cellText=cell.textContent;const cellId=cell.id;cell.innerHTML=`<input class="collection-input" type="text" value="${cellText}">`;cell.id=`${cellId}-input`;});console.log('row id when edit button has been clicked',row.id)
