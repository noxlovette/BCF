...
function editCollectionIngredient(event,id){const row=event.target.parentNode.parentNode;row.dataset.id=id;const cells=row.querySelectorAll('td.editable');cells.forEach((cell,index)=>{const cellText=cell.textContent;const cellId=cell.id;console.log('cellId before conversion',cellId)
let inputElement;if(cellId==='is_collection'){inputElement=document.createElement('input');inputElement.className="collection-input";inputElement.type="checkbox";if(cellText==='true'){inputElement.checked=true;}}else if(cellId==='amount'){inputElement=document.createElement('input');inputElement.className="collection-input";inputElement.type="number";inputElement.value=parseInt(cellText,10);}else{inputElement=document.createElement('input');inputElement.className="collection-input";inputElement.type="text";inputElement.value=cellText;}
inputElement.id=`${cellId}-input`;cell.innerHTML='';cell.appendChild(inputElement);console.log('inputId after conversion',inputElement.id)});const saveButton=createButton('Save','save',saveIngredient,id);const cancelButton=createButton('Cancel','cancel',fetchIngredients);const buttonCell=row.querySelector('.editing');buttonCell.appendChild(saveButton);buttonCell.appendChild(cancelButton);}
...
data[key]=input.type==='checkbox'?input.checked:input.value;console.log('data[key]',data[key])});$.ajax({url:`/collection/api/ingredient/${userId}/${id}/update/`,method:'PUT',headers:{'X-CSRFToken':getCookie('csrftoken'),'Content-Type':'application/json'},data:JSON.stringify(data),success:function(response){fetchIngredients();},error:function(error){console.error('Error saving ingredient:',error);}});}
