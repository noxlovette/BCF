...
function editCollectionIngredient(event,id){const row=event.target.parentNode.parentNode;row.dataset.id=id;const cells=row.querySelectorAll('td.editable');cells.forEach((cell,index)=>{const cellText=cell.textContent;const cellId=cell.id;let inputElement;if(cellId==='is_collection'){inputElement=`<input class="collection-input" type="checkbox" ${cellText === 'true' ? 'checked' : ''}>`;}else if(cellId==='amount'){inputElement=`<input class="collection-input" type="number" value="${parseInt(cellText, 10)}">`;}else{inputElement=`<input class="collection-input" type="text" value="${cellText}">`;}
cell.innerHTML=inputElement;cell.id=cellId;});const saveButton=createButton('Save','save',saveIngredient,id);const cancelButton=createButton('Cancel','cancel',fetchIngredients);const buttonCell=row.querySelector('.editing');buttonCell.appendChild(saveButton);buttonCell.appendChild(cancelButton);}
function saveIngredient(event){console.log('Saving ingredient...')
const id=event.target.dataset.id;console.log('id when save button has been clicked',id)
const row=document.querySelector(`tr[data-id="${id}"]`);const inputs=row.querySelectorAll('input.collection-input');const data={};inputs.forEach((input)=>{const key=input.id;data[key]=input.type==='checkbox'?input.checked:input.value;});$.ajax({url:`/collection/api/ingredient/${userId}/${id}/`,method:'PUT',headers:{'X-CSRFToken':getCookie('csrftoken'),'Content-Type':'application/json'},data:JSON.stringify(data),success:function(response){fetchIngredients();},error:function(error){console.error('Error saving ingredient:',error);}});}
