...
function editCollectionIngredient(event,id){const row=event.target.parentNode.parentNode;console.log('row id when edit button has been clicked',row.id)
console.log(row)
const cells=row.querySelectorAll('td.editable');cells.forEach((cell,index)=>{const cellText=cell.textContent;const cellId=cell.id;cell.innerHTML=`<input class="collection-input" type="text" value="${cellText}">`;cell.id=`${cellId}-input`;});console.log('row id when edit button has been clicked',row.id)
const saveButton=createButton('Save','save',saveIngredient,id);const cancelButton=createButton('Cancel','cancel',fetchIngredients);const buttonCell=row.querySelector('.editing');buttonCell.appendChild(saveButton);buttonCell.appendChild(cancelButton);}
function saveIngredient(event){const id=event.target.dataset.id;console.log('id when save button has been clicked',id)
