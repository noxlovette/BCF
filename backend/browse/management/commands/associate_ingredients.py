...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('cas1', type=str, help='CAS number of the first set of ingredients (cas1)')
        parser.add_argument('cas2', type=str, help='CAS number of the second set of ingredients (cas2)')
    ...
    def handle(self, *args, **options):
        ...
        if not ingredients_cas1.exists():
            self.stdout.write(self.style.ERROR(f'No ingredients found for cas1: {cas1}'))
            return
        if not ingredients_cas2.exists():
            self.stdout.write(self.style.ERROR(f'No ingredients found for cas2: {cas2}'))
            return
