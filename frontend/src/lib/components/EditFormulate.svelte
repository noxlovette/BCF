...
<form method="post" action="?/update" use:enhance>
  <div class="dark:bg-black *:dark:bg-black">
    ...
    <button on:click|preventDefault={addIngredient}> add new ing </button>
    <div id="ingredient holder" class="flex flex-col space-y-2 *:dark:bg-black">
      {#each $editedFormula.ingredients as ingredient, i}
        <input
          ...
          name="ingredients"
          placeholder="name"
          bind:value={$editedFormula.ingredients[i].common_name}
        />
        <input
          type="text"
          name="ingredients"
          placeholder="amount"
          bind:value={$editedFormula.ingredients[i].amount}
        />
        <input
          type="text"
          name="ingredients"
          placeholder="percentage"
          bind:value={$editedFormula.ingredients[i].percentage}
        />
        <input
          type="text"
          name="ingredients"
          placeholder="volatility"
          bind:value={$editedFormula.ingredients[i].volatility}
        />
