...
<button
  id="card-small"
  class="group flex flex-col rounded-lg items-start justify-between bg-white p-4 md:p-8 text-left shadow-lg transition-all hover:scale-105 hover:bg-rose-800 hover:text-rose-50 hover:shadow-lg dark:bg-stone-800"
...
>
  <div id="top-part" class="flex flex-row items-baseline justify-between w-full">
    <h1
      id="top-left"
      class="mr-8 hyphens-auto text-3xl max-w-[65%] overflow-clip font-bold tracking-tighter text-rose-800 group-hover:text-rose-50"
    >
      {ingredient.common_name}
    </h1>
    <h2
      id="top-right"
      class="ml-auto min-w-fit text-clip hidden md:flex opacity-60 group-hover:text-rose-200/60"
    >
      {ingredient.cas}
    </h2>
  </div>
  <div id="bottom-part" class="mt-2 md:mt-4 flex flex-row">
    <div id="bottom-left" class="mr-auto flex w-2/3 flex-col">
      ...
      <h2
        class="justify-start mt-auto lowercase opacity-60 group-hover:text-rose-200/60"
      >
