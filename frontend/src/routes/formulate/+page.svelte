...
<div class="flex flex-col min-h-screen" style="background: url('/assets/bg/bbblurry-formulate.svg') no-repeat center center fixed; background-size: cover;">
  ...
  <div id="app" class="flex flex-row items-stretch size-3/4 m-2 mt-0 lowercase font-light text-lime-950/90 dark:text-lime-200/80">

    
  <div id="sidebar" class="flex flex-col w-1/4 mr-auto bg-lime-700/20 rounded shadow p-2" style="background: rgb(77 124 15 / 0.2) url('/assets/bg/texture/nnnoise.svg') no-repeat; background-size:cover;">
    <h2 id="formula-header" class= "text-xl border-b-2 border-amber-500/80">my formulae</h2>
    <ul id="formulate-list" class= "overflow-y-hidden divide-y-2 divide-dashed divide-amber-500/60">
      {#each formulae as formula}
        <li id="formula-item" class="flex flex-col rounded hover:bg-lime-300/25 hover:shadow" on:dblclick={() => viewFormula(formula.id)}>
          <p id="formula-name">name: {formula.name}</p>
          <p id="formula-edit-time">edited: {formula.updated}</p>
        </li>
      {/each}
      <li id="formula-item" class="flex flex-col rounded hover:bg-lime-300/25 hover:shadow" on:dblclick={createFormula}>create new formula</li>
    </ul>
  </div>
  <div id="main-content" class="flex flex-row items-start flex-1 bg-lime-300/10 rounded shadow m-4 mb-0 mt-0 divide-x-2 divide-amber-700/60 dark:divide-amber-300/40">
    {#if formulaDetail}
      {#if formulaDetail.id == 0}
        <button
          class="m-2 p-2 bg-lime-600/50 hover:text-lime-900/25 rounded shadow"
          id="create-formula"
      ...
      {#if editing}
        <!-- Editing mode -->
        <div id="description-etc" class="flex flex-col mr-auto w-1/4 m-2 h-full divide-y-2 divide-lime-800/40 dark:divide-lime-300/60 space-y-4">
          <div>
            <h3 class="">Name: </h3>
            <input class="bg-white/20 dark:bg-black/20 text-lime-950/90 dark:text-lime-50/90"bind:value={editedFormula.name} />
          </div>  
            <div class="mt-2">
              <h3 >Description: </h3>
              <textarea class="bg-white/20 dark:bg-black/20 text-lime-950/90 dark:text-lime-50/90" bind:value={editedFormula.description} />
            </div>
            <div class="mt-2">
              <h3 >Time Edited: <h3>
                <p class="">{formulaDetail.updated}</p>
          </div>
          <div id="controls">
            <button on:click={saveChanges}>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 hover:text-amber-950/60 dark:hover:text-amber-300/60">
            ...
            {#if Array.isArray(editedFormula.ingredients)}
              ...
              <tr>
                ...
                <td>
                  <input
                    type="text"
                    class="size-full bg-white/20 dark:bg-black/20 text-lime-950/90 dark:text-lime-50/90"
                    bind:value={ingredient.ingredient}
                    on:input={(event) => handleInput(event, ingredient)}
                  />
                  <ul
                    id="ingredient-dropdown"
                    class="dropdown-menu bg-lime-700/50"
                    ...
                    {:else if Array.isArray($dropdownItems)}
                      {#each $dropdownItems as item (item.common_name)}
                        {#if $dropdownItems.length < 6}
                          <button
                            class="dropdown-item hover:text-lime-900/60 dark:hover:text-lime-300/60"
                  ...
                  <td>
                    <input type="number" bind:value={ingredient.amount} class="w-full h-full bg-white/20 dark:bg-black/20 text-lime-950/90 dark:text-lime-50/90"/>
                  </td>
                  <td>
                    <input type="number" bind:value={ingredient.percentage} class="w-full h-full bg-white/20 dark:bg-black/20 text-lime-950/90 dark:text-lime-50/90"/>
                  </td>
      ...
      {:else}
          <div id="description-etc" class="flex flex-col mr-auto w-1/4 m-2 h-full divide-y-2 divide-amber-800/25 dark:divide-amber-300/60 space-y-4">
            <div>
              <h3 class="">Name: </h3>
              <p class="">{formulaDetail.name}</p>
            </div>  
              <div class="pt-4">
              ...
              <div class="pt-4">
                <h3 class="">Time Edited: <h3>
                  <p class="">{formulaDetail.updated}</p>
              </div>
              <div id = "controls" class="pt-4">
                <button class=" hover:text-lime-700/80 dark:hover:text-lime-300/60" on:click={() => editFormula(formulaDetail)} title="edit the formula">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                  <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                  </svg>
                  </button>
                  <button class=" hover:text-lime-700/80 dark:hover:text-lime-300/60" on:click={() => deleteFormula(formulaDetail.id)} title="delete the formula">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                      <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                    </svg>
                  </button>
                  <button class=" hover:text-lime-700/80 dark:hover:text-lime-300/60" on:click={() => addAsCustom(formulaDetail)} title="add the formula to collection as ingredient">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                  ...
                  <button class=" hover:text-lime-700/80 dark:hover:text-lime-300/60" on:click={() => addTag(formulaDetail.id)} title='add tags to your formula'>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
          ...
          <table id="formula-ingredient-table" class="table-fixed text-left w-full m-2 pl-2">
            <thead>
              <tr>
                <th class="w-min p-2"> # </th>
                <th class="w-1/2 p-2"> Ingredient </th>
                <th class="w-1/6 p-2"> Volatility </th>
                <th class="w-1/6 p-2"> Amount </th>
