...
<div class="flex flex-col min-h-screen" style="background: url('/assets/bg/bbblurry-collect.svg') no-repeat center center fixed; background-size: cover;">
  ...
  <div class="mb-auto">
<div id = "app" class="flex flex-col content-center lowercase font-light w-full text-rose-900/90 dark:text-rose-200/90">
  ...
  <div id="tools" class="flex flex-row w-full justify-center space-x-2">
    <div id="search-bar" class="flex flex-row w-1/2 justify-between space-x-2">
      ...
      <button on:click={toggleTuneMenu}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 hover:text-amber-700/90 dark:hover:text-amber-300/90">
      ...
      {#if tuneMenuVisible}
      <div id="tune-menu" class="flex w-full p-2 bg-white/20 dark:bg-black/20 space-y-2 top-10 justify-start align-middle text-sm rounded">
        <div class="flex flex-row align-middle justify-start">
          <label class='flex items-center'> 
            Page Size:
            <input type="number" class='flex bg-white/20 dark:bg-black/20 pl-2 ml-4 w-20' min="1" bind:value={pageSize} on:change={updatePageSize}/>
          </label>
...
<button class="pl-2"on:click={saveEdit(editingObject)}>
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-24 hover:text-amber-700/90 dark:hover:text-amber-300/90">
    <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" />
  </svg>
</button>
  ...
  <table class="bg-blend-screen size-full table-fixed text-sm border-collapse overflow-hidden">
  <thead class="bg-pink-800/30 text-pink-900/90 dark:text-pink-400/90 space-x-2 h-8 text-xl tracking-widest align-middle text-center" style="background: rgb(157 23 77 / 0.3) url('/assets/bg/texture/nnnoise.svg') no-repeat; background-size:cover;">
    <tr>
  ...
  <tbody class="text-center divide-y divide-dashed divide-amber-700/70 dark:divide-amber-300/30">
    
    
  {#each ingredients as ingredient}
    <tr on:dblclick={() => toggleEdit(ingredient)} class="hover:bg-amber-700/10 dark:hover:bg-amber-300/10">
    ...
    <td class="align-middle m-4 p-4 {index === 0 ? 'text-2xl font-thin' : ''}">
      {#if isEditableField(ingredient.type, field.name) && editingRowId === ingredient.id}
        <textarea
          type="text"
          rows="3"
          class="bg-white/20 dark:bg-black/20 text-center flex w-full"
      ...
      {:else}
        <div id="icon container" class="flex align-middle justify-center">
          {@html ingredient[field.name] || `
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-8 hover:text-amber-700/90 dark:hover:text-amber-300/90">
